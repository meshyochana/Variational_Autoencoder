...
LATENT_DIM = 3  # Change this value to adjust latent space size

# Load the saved model
device = torch.device('cpu')  # Use CPU for simplicity
model = VariationalAutoencoder(latent_dim=LATENT_DIM)  # Ensure the model matches this dimension
model.load_state_dict(torch.load('vae_model_ldim_3.pth', map_location=device, weights_only=True))
model.eval()

@app.route('/latent_digits')
def get_latent_digits():
    # Could read from file or return it inline
    with open('latent_digits.json') as f:
        data = json.load(f)
    return jsonify(data)
